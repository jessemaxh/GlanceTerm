...
/**
 * Unit-tests the anti-sprawl decision in isolation: which tab removals trigger
 * cleanup. The git/fs engine has its own real-git integration tests; here the
 * WorktreeService is a spy so we assert only the lifecycle's branching:
 *   user-close + safe        → removeSet
 *   user-close + unsafe      → keep (toast only)
 *   app quit (recovery off)  → keep, no removeSet
 *   moved (session still open)→ keep, no removeSet
 *   unregistered tab         → no-op
 *
 * The map is keyed by the OUTER (top-level) tab — what tabRemoved$ emits — and
 * stores the inner leaf for the session-open move guard.
 */
function makeSet (branch: string): WorktreeSet {
    return { root: '/ws', isolatedRoot: '/managed/ws/' + branch, branch, repos: [] }
}
/** Inner-tab stub: a closed session (post-destroy) unless `open` given. */
function inner (open?: boolean): any {
    return open === undefined ? { session: undefined } : { session: { open } }
}
...
describe('WorktreeLifecycleService', () => {
    ...
    beforeEach(() => {
        ...
        svc = new WorktreeLifecycleService(
            { tabRemoved$ } as any,
            worktree as any,
    ...
    it('user-closes a CLEAN worktree tab → removeSet + "Tidied" toast', async () => {
        const outer = {} as any
        worktree.isSetSafeToRemove.mockResolvedValue(true)
        svc.register(outer, makeSet('agent/clean'), inner())
        ...
        expect(svc.branchForTab(outer)).toBeNull() // dropped from the map
    })

    it('user-closes a DIRTY worktree tab → kept, removeSet NOT called', async () => {
        const outer = {} as any
        worktree.isSetSafeToRemove.mockResolvedValue(false)
        svc.register(outer, makeSet('agent/dirty'), inner())
        tabRemoved$.next(outer)
        await flush()
    ...
    it('app QUITTING (recovery disabled) → keep even a clean set, never removeSet', async () => {
        const outer = {} as any
        worktree.isSetSafeToRemove.mockResolvedValue(true)
        svc.register(outer, makeSet('agent/quit'), inner())
    ...
    it('MOVED to a new window (session still open) → keep, never removeSet', async () => {
        const outer = {} as any
        worktree.isSetSafeToRemove.mockResolvedValue(true) // would be removed if we didn't guard
        svc.register(outer, makeSet('agent/move'), inner(true)) // PTY still alive
        tabRemoved$.next(outer)
        await flush()
        expect(worktree.isSetSafeToRemove).not.toHaveBeenCalled()
        expect(worktree.removeSet).not.toHaveBeenCalled()
    ...
    it('removal of an UNREGISTERED tab is a no-op', async () => {
    ...
    it('a removeSet failure is swallowed (never throws out of the subscription)', async () => {
        ...
        svc.register(outer, makeSet('agent/boom'), inner())
