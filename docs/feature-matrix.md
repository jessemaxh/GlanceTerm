...
| Active model shown next to the tag | 🧪 (hook `SessionStart` slug + transcript-`message.model` fallback in UsageTracker: a `resume` SessionStart carries an empty model, so resumed/auto-resumed tabs would otherwise show no chip — the transcript fallback restores it. The hook value wins when present, so a mid-session `/model` on a still-running tab updates only after a resume) | 🧪 (hook `.model` every event incl. SessionStart — source-confirmed; so the chip shows whenever the tab has fired any event. No transcript fallback: it would be dead code since the hook model is always present when a snapshot exists, and a zero-event resumed tab has no snapshot/transcript path to read) | 🧪 (no hook carries the model — `.llm_request.model` on `BeforeModel` is unsubscribed; surfaced instead from the saved-chat `messages[].model` via UsageTracker, last-wins so `/model` is tracked) | 🧪 (plugin `event.properties.info.modelID`) |
